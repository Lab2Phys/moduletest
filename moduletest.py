...
"""
    ...
    def _perform_analysis(self, selected_additional_branches):
        ...
        self._extract_state_space(q_to_letter, kvl_equations_new_vars, dq_symbols)
    ...
    def _extract_state_space(self, q_to_letter, kvl_equations_new_vars, dq_symbols):
        """Extract A and B matrices for dq/dt = Aq + Bu"""
        # Collect unique positive voltage source symbols
        voltage_symbols = []
        seen_vsyms = set()
        for val in self.voltage_source_map.values():
            sym = -val if str(val).startswith('-') else val
            if sym not in seen_vsyms:
                voltage_symbols.append(sym)
                seen_vsyms.add(sym)

        state_vars = [sp.Symbol(letter) for letter in q_to_letter.values()]
        n = len(state_vars)
        ...
        for eq_idx, eq in enumerate(kvl_equations_new_vars):
            if eq_idx >= n:
                break
            lhs = eq.lhs
            row_idx = None
            for i, dq_sym in enumerate(dq_symbols):
                if dq_sym in lhs.free_symbols and lhs.coeff(dq_sym) != 0:
                    row_idx = i
                    break
            if row_idx is None:
                continue
            try:
                sols = sp.solve(lhs, dq_symbols[row_idx])
                if not sols:
                    continue
                rhs = sols[0]
                for j, sv in enumerate(state_vars):
                    A_matrix[row_idx, j] = rhs.coeff(sv) if sv in rhs.free_symbols else 0
                for j, vs in enumerate(voltage_symbols):
                    B_matrix[row_idx, j] = rhs.coeff(vs) if vs in rhs.free_symbols else 0
            except Exception as e:
                print(f"⚠️ Warning: Could not extract coefficients from equation {eq_idx+1}: {e}")

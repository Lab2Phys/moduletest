...
"""
    ...
    def _setup_parameters(self):
        ...
        self.branches_for_additional = [b for b in self.all_branches
                                        if b not in self.capacitor_branches]
    ...
    def _extract_state_space(self, q_to_letter, kvl_equations, q_funcs, dq_funcs):
        ...
        Works directly with sympy Function derivatives (dq_funcs) so that
        coeff() and solve() operate on genuine sympy expressions — not
        string-based symbols like \\dot{x} which sympy cannot differentiate.
        """
        t = sp.Symbol('t')
        a = sp.Symbol('a')
        ...
        for val in self.voltage_source_map.values():
            sym = (-val) if val.could_extract_minus_sign() else val
            # unwrap any negation to get the bare Symbol
            atoms = sym.free_symbols
            for atom in atoms:
                if atom not in seen_vsyms:
                    voltage_symbols.append(atom)
                    seen_vsyms.add(atom)

        # ── ordered lists of state vars and their derivatives ──────────────
        q_names_sorted = sorted(q_to_letter.keys(), key=lambda x: int(x.split('_')[1]))
        dq_list  = [dq_funcs[q] for q in q_names_sorted]   # Derivative(x(t),t), ...
        q_list   = [q_funcs[q]  for q in q_names_sorted]   # x(t), y(t), ...
        # plain symbols for coefficient extraction after solving
        ...
        # keep track of which row has been filled
        filled_rows = set()

        for eq in kvl_equations:
            lhs = eq.lhs  # expression in terms of dq_funcs, q_funcs, voltage syms

            # find which derivative appears — must use atoms() not free_symbols
            # because Derivative objects do NOT appear in free_symbols
            deriv_atoms = lhs.atoms(sp.Derivative)
            row_idx = None
            for i, dq in enumerate(dq_list):
                if dq in deriv_atoms:
                    row_idx = i
                    break
            if row_idx is None or row_idx in filled_rows:
                continue
            filled_rows.add(row_idx)

            try:
                sols = sp.solve(lhs, dq_list[row_idx])
                if not sols:
                    continue
                rhs = sp.expand(sols[0])

                # substitute q(t) → plain symbol for coefficient extraction
                for q_func, s_sym in zip(q_list, state_sym):
                    rhs = rhs.subs(q_func, s_sym)

                # A coefficients (state variables)
                for j, sv in enumerate(state_sym):
                    A_matrix[row_idx, j] = sp.simplify(rhs.coeff(sv))

                # B coefficients (voltage sources)
                for j, vs in enumerate(voltage_symbols):
                    B_matrix[row_idx, j] = sp.simplify(rhs.coeff(vs))

            except Exception as e:
                print(f"⚠️ Could not extract state-space row {row_idx+1}: {e}")
        ...
        print("=" * 70)
        print("\nState vector q:")
        display(Latex(f"$q = {sp.latex(sp.Matrix(state_vars))}$"))
        print("\nInput vector u:")
        ...
        self._generate_matlab_file(A_matrix, B_matrix, state_vars, voltage_symbols)
